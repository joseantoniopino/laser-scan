...
class LaserRepository(ILaserRepository):
    def find_by_id(self, laser_id: str):
        ...
        if laser is None:
            raise InfrastructureException(status_code=404, code="laser_not_found",
                                          detail="Laser not found with target_id: {}".format(laser_id))
        laser_dict = {
            'id': str(laser[0]),
            'x': laser[1],
            'y': laser[2],
    ...
    def update_laser_last_shot(self, laser_id: str, last_shot: datetime):
        with session_scope() as session:
            ...
            if result.rowcount == 0:
                raise InfrastructureException(status_code=404, code="laser_not_found",
                                              detail="Laser not found with id: {}".format(laser_id))
